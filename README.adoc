...
The plugin archive published to the {uri-gradle-plugin-portal}[Gradle Plugin Portal] contains three plugins:
...
{uri-gradle-plugin-portal}plugin/com.github.rodm.teamcity-agent[agent] and
{uri-gradle-plugin-portal}plugin/com.github.rodm.teamcity-common[common] plugins.
...
`tests-support` dependencies to the `compile` and `testCompile` configurations. If the Java Plugin is not applied the plugin
provides only the tasks to package and publish the server side plugin archive if a plugin descriptor is defined.
