...
To use Docker to start and stop a TeamCity Server and Build Agent an environment can be configured with
the `useDocker()` function.
...
To start and stop a TeamCity Server and Build Agent using Docker requires Docker to be installed and running before
the running the environment tasks. The start server task uses the image `{uri-docker-teamcity-server}[jetbrains/teamcity-server]`
and the start agent task uses the image `{uri-docker-teamcity-agent}[jetbrains/teamcity-agent]`. The environment
`version` property is used to select an image by tag. The start server and start agent tasks also set name of the
containers to `teamcity-server` and `teamcity-agent`.

The default image and container names can be overridden by providing additional configuration to the
`useDocker` function as shown in the example below.
