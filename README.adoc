...
|4.0+
|Java 7 & later
|{uri-github-project}/blob/v1.2/README.adoc#using-the-plugin[README]
|v1.1
|3.0+
|Java 7 & 8
...
The `publishPlugin` task requires a username and password or a {uri-jetbrains-hub-token}[JetBrains Hub token] to
publish a plugin to the repository as shown in the following examples. As of August 2019 the use of `username` and
`password` for publishing is deprecated and no longer supported by the
...
The username and password or token can also be configured in the `publish` section of the `server` configuration as
