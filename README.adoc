...
image:{uri-github-status}?branch=v1.5.x["Build Status", link="{uri-github-actions}"]
...
The support for inheriting properties from the root project is now deprecated and will be removed in version 2.0.
This functionality can be disabled by setting the Gradle property `teamcity.disableInheritedProperties` to `true`.
The recommended way to share or set common configuration between projects is to use
https://docs.gradle.org/current/userguide/custom_plugins.html#sec:convention_plugins[convention plugins].
For examples see the `build-logic` directory in the following projects
https://github.com/rodm/teamcity-gradle-init-scripts-plugin/[Gradle Init Scripts] plugin and
https://github.com/rodm/teamcity-jvm-monitor-plugin/[JVM Monitor] plugin.
